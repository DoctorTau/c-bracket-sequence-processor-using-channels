...
Для решения задачи нужно было использовать 3 процесса, которые будут общаться между собой через неименнованные каналы.
...
Для решения задачи нужно было исльповать 3 процесса, которые будут общаться между собой через именнованные каналы.
...
Для решения задачи нужно было ислозовать 2 программы, которые будут общаться между собой через именнованный канал.

### Схема работы

1. Программа создает имменованный канал для общения и ждет передачи в нее строки для обработки.
2. Вторая программа занимается чтением и записью в файлы, а так же закрывает канал по завершении.

Программы запуска.тся последовательно в параллельный терминал.
...
Исходный код программы логики находится в папке [src](./8-score/brackets_checker/src/).
В папке [bin](./8-score/brackets_checker/bin/) находится исполняемый файл программы.

Исходный код программы чтения и записи находится в папке [src](./8-score/file_io/src/).
В папке [bin](./8-score/file_io/bin/) находится исполняемый файл программы.

Пример запуска программы из дирректории [8-score](./8-score/):

```bash
./brackets_checker/bin/brackets_checker


# В другом терминале
./file_io/bin/file_io ../Test/test1.txt output.txt
```

### Результаты

Результаты тестов находятся в папке [test_results](./8-score/test_results/).
